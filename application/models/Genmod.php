...
class Genmod extends CI_Model
{
    ...
    */
    ...
    public function update_last_seen_time($id, $table_name)
    {
        ...
        $this->db->platform() == "sqlite3" ? $this->db->set('last_seen', "datetime('now')", FALSE) : $this->db->set('last_seen', "NOW()", FALSE);
    ...
    */
    ...
    public function getPaymentMethods($year)
    {
        if ($this->db->platform() == "sqlite3") {
            ...
            $this->db->select('modeOfPayment, COUNT(ref) as total');
            $this->db->where('YEAR(transDate)', $year);
